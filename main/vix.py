...
# Calculating the daily exponential weighted average
uvxy_weighted_daily_returns = []
daily_smoothing = 2

daily_weightings = []
for x in range(0, len(uvxy_historical_daily_returns)):
    daily_weightings.append(daily_smoothing/(x+1))

daily_weightings = sorted(daily_weightings)

for index in range(0,len(uvxy_historical_daily_returns)):
    uvxy_weighted_daily_returns.append(uvxy_historical_daily_returns[index] * (daily_weightings[index] + 1))

uvxy_daily_ewa = sum(uvxy_weighted_daily_returns) / len(uvxy_weighted_daily_returns)
...
# Calculating the weekly exponential weighted average
uvxy_weighted_weekly_returns = []
weekly_smoothing = 2

weekly_weightings = []
for x in range(0, len(uvxy_historical_weekly_returns)):
    weekly_weightings.append(weekly_smoothing/(x+1))

weekly_weightings = sorted(weekly_weightings)

for index in range(0,len(uvxy_historical_weekly_returns)):
    uvxy_weighted_weekly_returns.append(uvxy_historical_weekly_returns[index] * (weekly_weightings[index] + 1))

uvxy_weekly_ewa = sum(uvxy_weighted_weekly_returns) / len(uvxy_weighted_weekly_returns)
...
# Calculating the monthly exponential weighted average
uvxy_weighted_monthly_returns = []
monthly_smoothing = 2

monthly_weightings = []
for x in range(0, len(uvxy_historical_monthly_returns)):
    monthly_weightings.append(monthly_smoothing/(x+1))

monthly_weightings = sorted(monthly_weightings)

for index in range(0,len(uvxy_historical_monthly_returns)):
    uvxy_weighted_monthly_returns.append(uvxy_historical_monthly_returns[index] * (monthly_weightings[index] + 1))

uvxy_monthly_ewa = sum(uvxy_weighted_monthly_returns) / len(uvxy_weighted_monthly_returns)
...
for price in range(len(prices_used) - time_period - moving_time_period, len(prices_used)):
    ...
    if len(prices_used[i - moving_time_period: i]) == 0:
        ...
        logger.info(SMA_30_DAY[-1] > EMA_30_DAY[-1])
